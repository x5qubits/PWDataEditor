...
namespace AndBurn.DDSReader
{
	public class DDSImage : IDisposable
	{
		...
		private void Parse(BinaryReader reader, Utils.PixelFormat pixel)
		{
			...
			if(ReadHeader(reader, ref header))
			{
				...
				{
                    byte[] rawData = null;
                    if (pixel == Utils.PixelFormat.UNKNOWN)
                    {
                        rawData = Decompressor.Expand(header, data, pixelFormat);
                    }else
                    {
                        rawData = Decompressor.Expand(header, data, pixel);
                    }
